...
if (!is_dir($tmpDir)) {
    ...
    saveData('voh_fm', $liveToken, $liveExpiration, $tmpDir);
} else {
    if (!file_exists($tmpDir . '/voh_fm_token.txt') && !file_exists($tmpDir . '/voh_fm_expiration.txt')) {
        // 获取汉声 FM 台 M3U8 地址，1 是 FM，2 是 AM
        $m3u8Link = parseVoh(1);
        // 内容写入到文件
        $liveToken = parseUrl($m3u8Link, 'token');
        ...
        if (!validTime(readData($tmpDir, 'voh_fm', 'expiration'))) {
            // 重新获取 M3U8 地址，1 是 FM，2 是 AM
            $m3u8Link = parseVoh(1);
            // 内容写入到文件
            $liveToken = parseUrl($m3u8Link, 'token');
            $liveExpiration = parseUrl($m3u8Link, 'expiration');
            ...
            // 从文件读取原有 Token
            $liveToken = readData($tmpDir, 'voh_fm', 'token');
            $m3u8Link = 'https://vohradiow-hichannel.cdn.hinet.net/live/RA000076/chunklist.m3u8?token=' . $liveToken . '&expires=' . readData($tmpDir, 'voh_fm', 'expiration');
        }
    }
}
...
echo str_replace('media_', 'https://vohradiow-hichannel.cdn.hinet.net/live/RA000076/media_', $streamContents);
